...
/**
 * Creates the query object and adds the expression object to query if available.
 *
 ...
 *
 * @returns {Object} query - query object used for filtering.
 */
function expressionQuery(expression, field, value) {
  var query = {}
  if (!expression) {
    query[field] = value;
    return query;
  ...
  console.log("------expressionQuery[expression]: ", expressionQuery)
  query[field] = expressionQuery;
  return query;
}

function number(expression, field, value) {
  var num = Number(value);
  if (!Number.isNaN(num)) {
    return expressionQuery(expression, field, value);
  }
}

function objectId(expression, field, value) {
  try {
    return expressionQuery(expression, field, ObjectId(value));
  } catch (err) { }
}

function bool(expression, field, value) {
  if (value.toLowerCase() === 'true') {
    return expressionQuery(expression, field, true);
  } else if (value.toLowerCase() === 'false') {
    return expressionQuery(expression, field, false);
  }
}
...
var convertors = {
  ...
  "Any": function (value, field, expression) {
    var x = casters.reduce(function (acc, caster) {
      acc.push(caster(expression, field, value));
      console.log("--------acc: ", acc);
      return acc;
    }, [expressionQuery(expression, field, value)])
...
function convertQueryType(type, field, expression) {
  if (!convertors[type.type]) {
    ;
    return expressionQuery(expression, field, type.value);
  }
  return convertors[type.type](type.value, field, expression);
}
      ...
      for (field in fields) {
        if (fields.hasOwnProperty(field)) {
          if (fields[field].type === "Any") {
            query['$or'] = convertQueryType(fields[field], field);
          } else {
            query = convertQueryType(fields[field], field);
            console.log("--------------query with eq expression: ", query);
          }
        }
      }
    }
    ...
    for (field in  fields) {
      if (fields.hasOwnProperty(field)) {
        ...
        if ('undefined' !== typeof query[field]) {
          ...
          query['$or'] = convertQueryType(fields[field], field, expression);
        } else {
          query = convertQueryType(fields[field], field, expression);
        }
      }
    }
  }
