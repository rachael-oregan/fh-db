 ...
 */
function expressionQuery(expression, value) {
  var expressionQuery = {};
  expressionQuery[expression] = value;
  return expressionQuery[expression];
}

// contains functions to convert the query value based on the query type.
var convertors = {
  "Date": function (value) {
  ...
  "Any": function (value, field, expression) {
    ...
    arr.push(query);
    try {
      query[field] = expression ? expressionQuery(expression, value) : ObjectId(value);
      arr.push(query);
    } catch (err) { }
    var num = Number(value);
    if (!Number.isNaN(num)) {
      query[field] = expression ? expressionQuery(expression, num) : num;
      arr.push(query);
    }

    if (value.toLowerCase() === 'true') {
      query[field] = expression ? expressionQuery(expression, true) : true;
      arr.push(query);
    } else if (value.toLowerCase() === 'false') {
      query[field] = expression ? expressionQuery(expression, false) : false;
      arr.push(query);
    }
    return arr;
  }
...
function convertQueryType(type, field, expression) {
  if (convertors[type.type] === undefined) {
    return type.value;
  } else {
      ...
      for (field in fields) {
        if (fields.hasOwnProperty(field)) {
          if (fields[field].type === "Any") {
            query['$or'] = convertQueryType(fields[field], field, undefined);
          } else {
            query[field] = convertQueryType(fields[field], field, undefined);
          }
        }
      }
    }
    ...
    for (field in  fields) {
      if (fields.hasOwnProperty(field)) {
        ...
        if ('undefined' !== typeof query[field]) {
          ...
          queryField[expression] = convertQueryType(fields[field], field, expression);
          query[field] = queryField;
        }
      }
    }
